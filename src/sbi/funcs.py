...
def crop_face(img, landmark=None, bbox=None, margin=False, crop_by_bbox=True, abs_coord=False, only_img=False, phase='train', seed=None):
    ...
    if margin:
        ...
        if (seed == None):
            ...
            rand = np.random.default_rng(seed=seed).random
        w0_margin *= (rand()*0.6+0.2)  # np.random.rand()
        w1_margin *= (rand()*0.6+0.2)  # np.random.rand()
        h0_margin *= (rand()*0.6+0.2)  # np.random.rand()
        h1_margin *= (rand()*0.6+0.2)  # np.random.rand()
    else:
        w0_margin *= 0.5
        w1_margin *= 0.5
        h0_margin *= 0.5
    ...
    # recalculate landmark coordinates
