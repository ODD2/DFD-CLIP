...
def dynamic_blend(source, target, mask, ratio):
    mask_blured = get_blend_mask(mask)
    blend_ratio = ratio
...
def get_blend_mask(mask):
    ...
    kernel_1 = np.random.randint(2, 13) * 2 + 1  # random.randrange(5, 26, 2)
    kernel_1 = (kernel_1, kernel_1)
    kernel_2 = np.random.randint(2, 13) * 2 + 1
...
def get_alpha_blend_mask(mask):
    ...
    kernel_idxs = np.random.choice(range(len(blend_list)), 2)  # random.choices(range(len(kernel_list)), k=2)
    blend_ratio = np.random.choice(blend_list)  # blend_list[random.sample(range(len(blend_list)), 1)[0]]
